/**
 * 验证视频 RGB 通道参数生效（2026-08-12；2026-09 更新：视频链路唯一模式为单层色调映射 transform）
 * /video-frame mode=transform 下，不同 rgbAdjustment 应改变输出通道均值
 */
...
async function main() {
    ...
    console.log('\n=== mode=transform（单层色调映射）===')
    const neutral = await httpJson('POST', '/video-frame', {
        inputPath: FRAME, settings: { ...base, rgbAdjustment: { red: 1, green: 1, blue: 1 } }, peak: 8, mode: 'transform'
    })
    const boosted = await httpJson('POST', '/video-frame', {
        inputPath: FRAME, settings: { ...base, rgbAdjustment: { red: 2, green: 0.5, blue: 1 } }, peak: 8, mode: 'transform'
    })
    ...
    console.log((ok ? '✅' : '❌') + ' transform（单层色调映射）RGB 生效（红↑、绿↓）')
