...
__global__ void kGainMapPhase1(const uchar4* __restrict__ in, float* __restrict__ gain,
    ...
    if (tid == 0) partialMin[blockIdx.x] = sdata[0];
    // max 归约
    __syncthreads();
    sdata[tid] = lmax;
    __syncthreads();
    ...
    if (tid == 0) partialMax[blockIdx.x] = sdata[0];
}
...
__global__ void kHdrTransformPhase1(const uchar4* __restrict__ in, float3* __restrict__ linear,
    ...
    if (tid == 0) partialLum[blockIdx.x] = sdata[0];
}
...
__global__ void kHdrTransformPhase2(const float3* __restrict__ linear, uchar4* __restrict__ out,
    ...
    r *= totalExposure; g *= totalExposure; b *= totalExposure;
...
static void throwJni(JNIEnv* env, const char* msg) {
    ...
    else fprintf(stderr, "[hdr_gpu_jni] %s\n", msg);
}
...
Java_com_hdrconverter_HdrGpuJni_nativeSrgbToDisplayP3(
    ...
    if (e == cudaSuccess) e = cudaMalloc(&dOut, (size_t)n * 4);
    if (e == cudaSuccess) e = cudaMemcpy(dIn, src, (size_t)n * 4, cudaMemcpyHostToDevice);
    if (e == cudaSuccess) {
        kSrgbToP3<<<numBlocks(n), THREADS>>>(dIn, dOut, n);
        e = cudaGetLastError();
        if (e == cudaSuccess) e = cudaDeviceSynchronize();
    }
    if (e == cudaSuccess) e = cudaMemcpy(dst, dOut, (size_t)n * 4, cudaMemcpyDeviceToHost);
...
Java_com_hdrconverter_HdrGpuJni_nativeComputeGainMap(
    ...
    cudaError_t e = cudaMalloc(&dIn, (size_t)n * 4);
    if (e == cudaSuccess) e = cudaMalloc(&dGain, (size_t)n * sizeof(float));
    if (e == cudaSuccess) e = cudaMalloc(&dGm8, (size_t)n);
    if (e == cudaSuccess) e = cudaMalloc(&dMin, (size_t)blocks * sizeof(float));
    if (e == cudaSuccess) e = cudaMalloc(&dMax, (size_t)blocks * sizeof(float));
    if (e == cudaSuccess) e = cudaMemcpy(dIn, src, (size_t)n * 4, cudaMemcpyHostToDevice);
    if (e == cudaSuccess) {
    ...
    if (e == cudaSuccess) {
        ...
        if (e == cudaSuccess) e = cudaMemcpy(hMax.data(), dMax, (size_t)blocks * sizeof(float), cudaMemcpyDeviceToHost);
        for (int b = 0; b < blocks && e == cudaSuccess; b++) {
            if (hMin[b] < gmin) gmin = hMin[b];
            if (hMax[b] > gmax) gmax = hMax[b];
        }
    }
    ...
    if (e == cudaSuccess) {
        ...
        if (dMinB < 0.25) dMinB = 0.25;
        if (dMinB > 1.0) dMinB = 1.0;
        minBoost = dMinB;
    ...
    if (e == cudaSuccess) e = cudaMemcpy(gm8, dGm8, (size_t)n, cudaMemcpyDeviceToHost);
    if (e == cudaSuccess) {
        minmax[0] = minBoost;
        minmax[1] = maxBoostActual;
    }

    if (dMax) cudaFree(dMax);
    if (dMin) cudaFree(dMin);
    if (dGm8) cudaFree(dGm8);
...
Java_com_hdrconverter_HdrGpuJni_nativeApplyHdrTransform(
    ...
    cudaError_t e = cudaMalloc(&dIn, (size_t)n * 4);
    if (e == cudaSuccess) e = cudaMalloc(&dLinear, (size_t)n * sizeof(float3));
    if (e == cudaSuccess) e = cudaMalloc(&dOut, (size_t)n * 4);
    if (e == cudaSuccess) e = cudaMalloc(&dLum, (size_t)blocks * sizeof(float));
    if (e == cudaSuccess) e = cudaMemcpy(dIn, src, (size_t)n * 4, cudaMemcpyHostToDevice);
    if (e == cudaSuccess) {
        size_t shmem = THREADS * sizeof(float);
    ...
    if (e == cudaSuccess) {
        ...
        double sum = 0.0;
        for (int b = 0; b < blocks; b++) sum += hLum[b];
        double mean = sum / n;
